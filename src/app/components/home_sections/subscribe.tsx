...
import { subscribeToNewsletter } from "@/app/services/neewsletter";
...
export function SubscribeSection({ className }: SubscribeSectionProps) {
  ...
  return (
    <div className={`${className} responsive !mb-24`}>
      <div
        style={{ backgroundImage: "" }}
        className="rounded-md bg-[linear-gradient(289deg,#58126A_-40.56%,#F6B2E1_119.32%)] px-5 sm:px-10 py-6 flex gap-10 justify-between items-center flex-col md:flex-row mx-auto my-0 max-w-[1024px]"
      >
        ...
        <div>
          <div className="md:basis-1/2 w-full border border-white/60 rounded flex justify-center items-center p-1 gap-1">
            ...
            <Button
              isLoading={isLoading}
              className="text-black bg-white hidden xs:block !rounded-md"
          ...
          <Button
            isLoading={isLoading}
            className="text-black bg-white w-full flex justify-center xs:hidden !rounded-md mt-3"
            onClick={handleSubmit}
            loadingClass="text-black"
          >
            <span className="text-black text-sm font-semibold !rounded">
