...
export default async function SubscriptionPlanPage() {
  ...
  async function updateUserPlan(planId: string) {
    ...
    try {
      ...
      redirect("/subscription-plan?planUpdatedStatus=success");
    } catch (err: any) {
      console.log(err);
      redirect("/subscription-plan?planUpdatedStatus=success");
    }
  ...
  return (
    <div className="py-10">
      <PlanUpdated currentPlanName={userPlan} />
      ...
      <div className="my-2 grid grid-cols-3 gap-7">
        {plans.map((plan, id) => (
          <div
            key={id}
            className="border relative border-black rounded-lg flex gap-5 justify-between flex-col px-5 py-5"
          >
            {plan.id === userPlan && (
              <div className="bg-[#1c1c1c]/10 w-fit top-9 absolute -translate-y-1/2 rounded-lg">
                ...
                <h2 className="text-primary font-bold text-3xl">
                  {plan.discounted_price === 0
                    ...
                    : `$${(plan.discounted_price || 0) / 100}/${
                        plan.subscription_interval === "monthly" ? "mo" : "yr"
                      }`}
                </h2>
